...
const handleRead = async () => {
  try {
    const response = await axios.get(BASE_URL, { params: { _limits: 3 } })
...
const handleCreate = async () => {
  ...
  try {
    const payload = { title: textInput.value, body: '셈플 내용', userId: 1 }
    ...
    <ul class="item-list">
      <li v-for="item in items" :key="item.id" class="item-card">
        ...
        <div class="btn-group">
          ...
          <button @click="hadnleDelete(item.id)" class="btn-delete">DEL (삭제)</button>
        </div>
      </li>
    </ul>
  </div>
